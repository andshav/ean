  ...
  const generateCodes = () => {
    ...
    while (generatedCodes.length < quantity) {
      ...
      const baseCode = mask.replace(
        /x/g,
        (match, idx, str) => randomPart.split("")[idx % randomPart.length]
      );
  ...
  return (
    <VStack spacing={4} mt={5}>
      ...
      <NumberInput
        min={1}
        value={quantity}
        onChange={(_, valueAsNumber) => setQuantity(valueAsNumber || 1)}
      >
        <NumberInputField placeholder="Enter the number of codes" />
      </NumberInput>
